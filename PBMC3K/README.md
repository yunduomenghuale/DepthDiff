# PBMC3K DepthDiff Experiment

This folder contains the final PBMC3K benchmark used for low-depth single-cell RNA-seq enhancement.

## Final Setup

- Dataset: PBMC3K 10x Genomics count matrix.
...
- `data_processing.py`: downloads PBMC3K, applies QC, selects HVGs, and creates the fixed-UMI benchmark.
...
- `data/processed/pbmc3k_fixed_umi_pairs.npz`: paired high-depth and low-depth matrices.
...
The filtered PBMC3K count matrix is treated as pseudo-high-depth data. Each cell is downsampled to fixed UMI targets corresponding to 25%, 50%, and 75% of the pseudo-high-depth median library size in HVG space.
