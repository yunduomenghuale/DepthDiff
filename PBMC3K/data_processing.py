...
def download_pbmc3k() -> None:
    ensure_dirs()
    if config.PBMC3K_EXTRACTED.exists():
        print(f"Found extracted PBMC3K data at {config.PBMC3K_EXTRACTED}")
        return
    if not config.PBMC3K_TAR.exists():
        print(f"Downloading PBMC3K from {config.PBMC3K_URL}")
        urllib.request.urlretrieve(config.PBMC3K_URL, config.PBMC3K_TAR)
    print("Extracting PBMC3K archive")
    with tarfile.open(config.PBMC3K_TAR, "r:gz") as tar:
...
def select_highly_variable_genes(counts: sparse.csr_matrix, n_top: int) -> np.ndarray:
    log_norm = normalize_log1p(counts)
    means = log_norm.mean(axis=0)
    variances = log_norm.var(axis=0)
...
def preprocess() -> None:
    ...
    matrix_path = config.PBMC3K_EXTRACTED / "matrix.mtx"
    genes_path = config.PBMC3K_EXTRACTED / "genes.tsv"
    barcodes_path = config.PBMC3K_EXTRACTED / "barcodes.tsv"
