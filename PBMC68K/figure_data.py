...
def main():
    ...
    # For the 50%-depth pred/target matrices (density figure), save a random
    # subsample of up to MAX_CELLS_FOR_MATRIX test cells so the npz stays small
    # on large datasets. For datasets with fewer test cells this keeps all of
    # them. The same cell subset is used across all methods + target.
    rng = np.random.default_rng(config.SEED)
    n_test = target.shape[0]
    sub = (rng.choice(n_test, size=MAX_CELLS_FOR_MATRIX, replace=False)
           if n_test > MAX_CELLS_FOR_MATRIX else np.arange(n_test))
    ...
    for depth in config.DEPTHS:
        ...
        for name, pred in preds.items():
            ...
            if pct == 50:
                save[f"pred_{name}_50"] = pred[sub].astype(np.float32)
        if pct == 50:
            save["target_50"] = target[sub].astype(np.float32)
